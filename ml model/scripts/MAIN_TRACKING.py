...
def count_total():
    while True:
        ...
        if total_humans_in == total_cards_in:
            ...
            #bytearray_data = np.array(screenshot, dtype=np.uint8).tobytes()
            #upload_data(wearing_card, bytearray_data)

            '''quality = 20
            encode_params = [int(cv2.IMWRITE_JPEG_QUALITY), quality]
            retu, compressed_image = cv2.imencode('.jpg', screenshot, encode_params)
            if not retu:
                raise Exception("Error encoding image")
            compressed_bytearray = bytearray(compressed_image.tobytes())
            upload_data(wearing_card, compressed_bytearray)'''
            ...
            upload_data(wearing_card, compressed_bytearray)'''
            cv2.imwrite('screenshot.jpg', screenshot)
            #path = os.path.join('.', 'scripts\\screenshot.jpg')
            path = "D:\\Apps\\yolo3\\scripts\\screenshot.jpg"
            upload_data(wearing_card, path)
            os.remove(path)
